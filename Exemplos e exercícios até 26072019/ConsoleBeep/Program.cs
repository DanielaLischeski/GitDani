...
namespace ConsoleBeep
{
    static class Tetris
    {
        ...
        static void Main()
        {
            //Aqui nesta parte ele pega a hora(24h) e minutos
            var dateVerify = DateTime.Now.ToString("HH:mm");

            Console.WriteLine("informe sua fileira");
            //Pede a informação da fileira que irá executar
            var fileira = Console.ReadLine(); //Pega o número digitado

            Console.WriteLine("Informe a hora de inicio no formato HH:mm");
            //Estamos assinando nossa variável de comparação
            var timeStart = Console.ReadLine();

            //Laço de repetição que fica mudando enquanto não for a hora marcada
            while (dateVerify != timeStart)
            {
                //Atualiza o valor da variável "dateVerify" com a hora e minutos atuais
            ...
            switch (fileira)
            {
                ...
                case "1": {
                        //Executa a chamada de acordo com o número escolhido
                ...
                case "10": { Fileira10(startTimerIcrement); } break;
                    //Em caso de não ser informado uma string válida para nosso menu, ele executa esse cara
